...
GLTFDocument GLTFTexturePackingUtils::PackMaterialForWindowsMR(const IStreamReader& streamReader, const GLTFDocument& doc, const Material& material, TexturePacking outputPacking, const std::string& outputDirectory)
{
    GLTFDocument outputDoc(doc);

    // No packing requested, return copy of document
    if (outputPacking == TexturePacking::None)
    {
        return outputDoc;
    }
    ...
    if (outputPacking & TexturePacking::OcclusionRoughnessMetallic && (hasMR || hasOcclusion))
    {
    ...
    if (outputPacking & TexturePacking::RoughnessMetallicOcclusion && (hasMR || hasOcclusion))
    {
...
GLTFDocument GLTFTexturePackingUtils::PackAllMaterialsForWindowsMR(const IStreamReader& streamReader, const GLTFDocument & doc, TexturePacking outputPacking, const std::string& outputDirectory)
{
    GLTFDocument outputDoc(doc);
    ...
    {
        outputDoc = PackMaterialForWindowsMR(streamReader, outputDoc, material, outputPacking, outputDirectory);
    }
